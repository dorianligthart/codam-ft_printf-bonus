...
/*   Updated: 2024/01/11 20:07:34 by doligtha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// static void	pasteitem(t_comp *node, char *tmp, int *itemlen)
// {
// 	if (node->conv && node->conv->conversion)
// 	{
...
int	ft_printf_printcomp(int fd, t_comp *origin)
{
	...
	while (node)
	{
		pasteitem(node, tmp, &node->itemlen);
		tmp += node->itemlen;
		node = node->next;
	}
